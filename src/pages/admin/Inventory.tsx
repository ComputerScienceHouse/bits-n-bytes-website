...
export default function InventoryPage() {
  ...
  const fetchItems = useCallback(async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_PUBLIC_API_ENDPOINT}/get_items`, {
        headers: apiHeaders,
      });
  ...
  const fetchAllShelves = useCallback(async () => {
    setLoadingAll(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_PUBLIC_API_ENDPOINT}/shelves`, {
        headers: apiHeaders,
      });
      if (!res.ok) throw new Error();
      const data: { shelf_id: number | string; slots: { slot_id: number; items: SlotItem[] }[] }[] =
        await res.json();
  ...
  const fetchShelf = useCallback(async (shelfId: string) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_PUBLIC_API_ENDPOINT}/shelf/${shelfId}`, {
        headers: apiHeaders,
      });
      const contents = emptyShelf();
      if (res.ok) {
  ...
  const addItemToSlot = async (shelfId: string, slotId: number) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_PUBLIC_API_ENDPOINT}/shelf/${shelfId}/slot/${slotId}`,
        {
          method: "POST",
          headers: apiHeaders,
          body: JSON.stringify({
            item_id: Number(state.itemId),
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <div className="p-2 rounded-lg bg-primary/20 border border-purple-500/30">
                <Package className="h-5 w-5 text-primary" />
              </div>
              <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Inventory</h1>
            </div>
            <p className="text-sm text-muted-foreground pl-1">
              {shelfIds.length} shel{shelfIds.length !== 1 ? "ves" : "f"} &middot; {totalItems} item
              slot{totalItems !== 1 ? "s" : ""} filled
            </p>
          </div>
          <Button
            variant="outline"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-white/10">
                  {Array.from({ length: SLOTS_PER_SHELF }, (_, j) => {
                    ...
                    return (
                      <div
                        key={slotId}
                        className="bg-card p-4 flex flex-col gap-3 border rounded-lg"
                      >
                        {/* Slot label */}
                        ...
                        {/* Items list */}
                        <div className="flex-1 space-y-2 min-h-15">
                          {slotItems.length === 0 ? (
                            <div className="flex items-center justify-center h-full min-h-12 rounded-lg border border-dashed border-white/10 text-xs text-muted-foreground">
                              Empty
                            </div>
                          ) : (
                            slotItems.map((item) => {
                              ...
                              return (
                                <div
                                  ...
                                  className="flex items-center gap-2 bg-white/5 border rounded-lg px-3 py-2 group"
                                >
                        ...
                        <div className="pt-3 border-t border-white/10 space-y-2">
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button
                              size="sm"
                              className="h-9 flex-1 text-sm bg-primary/80 hover:bg-primary/80 border-0"
        ...
        <div className="rounded-2xl border border-dashed border-white/15 p-4 sm:p-5">
          ...
          <div className="flex gap-3">
            <Input
              ...
              className="h-10 max-w-45 border-white/15 bg-white/5"
            />
            <Button
              onClick={addShelf}
              disabled={!newShelfInput.trim()}
              className="h-10 bg-primary/80 hover:bg-primary/80 border-0 gap-2"
            >
